...
link1_status=$(curl -Is https://ia803109.us.archive.org/27/items/WS2019_trial/WS2019.iso | grep HTTP | cut -f2 -d" " | head -1)
...
if [ $link1_status = "200" ] ; then 
	sudo wget -O /mediabots/WS2019.ISO http://205.185.126.191/web/iso/server2019.iso
elif [ $link2_status = "200" -o $link2_status = "301" -o $link2_status = "302" ] ; then 
	sudo wget -P /mediabots http://205.185.126.191/web/iso/server2019.iso
else
